...
function void warn_target(obj this, obj target) {
	string msg;
	list messages;
	string bark_str;
	int dir;
	string notoriety_str;
	int rep;
	if (!getCompileFlag(0x01)) {
		rep = getNotoriety(target);
	} else {
		rep = getKarma(target);
	}
	notoriety_str = rep;
	int rep_level;
	if (!getCompileFlag(0x01)) {
		rep_level = (0x00 - (getNotoriety(target) / 0x0A));
	} else {
		rep_level = getKarmaLevel(target);
	}
	...
	int msg_index = random(0x00, 0x0C) + (rep_level / 0x02);
	msg = messages[msg_index];
	bark_str = getName(target) + "! " + msg;
	dir = getDirectionInternal(getLocation(this), getLocation(target));
	faceHere(this, dir);
	bark(this, bark_str);
...
function void attack_target(obj this, obj target) {
	string msg;
	list messages;
	string bark_str;
	int dir;
...
trigger callback(0x63) {
	...
	string region_prefix = "city";
	loc my_loc = getLocation(this);
	int in_region = 0x00;
	in_region = isInRegionWithPrefix(region_prefix, my_loc);
	if (!in_region) {
		region_prefix = "justice";
		in_region = isInRegionWithPrefix(region_prefix, my_loc);
	}
	if (!in_region) {
		region_prefix = "guard";
	...
	if (!in_region) {
		doLocAnimation(my_loc, 0x3728, 0x0A, 0x0A, 0x00, 0x00);
		sfx(my_loc, 0x01FE, 0x00);
		deleteObject(this);
	}
...
trigger give {
	...
	if (hasObjVar(givenobj, "nameVar")) {
		...
		string player_name = getObjVar(givenobj, "nameVar");
		setObjVar(giver, "bountyPlayerName", player_name);
