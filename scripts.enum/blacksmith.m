...
trigger targetobj {
	...
	if (isWeapon(usedon) && hasResource(usedon, resourceTypeToId("metal"))) {
		...
		int result;
		int success = testAndLearnSkill(user, SKILL_BLACKSMITHY, difficulty, 0x32);
		max_hp--;
		cur_hp--;
		if (cur_hp < 0x01) {
...
trigger use {
	...
	int i;
	if (has_anvil_and_forge(user)) {
		int skill_level = getSkillLevelReal(user, SKILL_BLACKSMITHY);
...
function void craft_item() {
	...
	int success = testAndLearnSkill(crafter, SKILL_BLACKSMITHY, getArrayIntElem(0x00, 0x05, recipe_index), 0x32);
	obj new_item;
	if (success <= 0x00) {
