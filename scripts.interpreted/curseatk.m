...
trigger equip {
	...
	if (is_bow == 0x01) {
		loseSkillLevel(equippedon, 0x1F, skill_penalty);
	} else {
		loseSkillLevel(equippedon, 0x1B, skill_penalty);
	}
	return(0x01);
}
...
trigger unequip {
	skill_penalty = 0xC8;
	if (is_bow == 0x01) {
		addSkillLevel(unequippedfrom, 0x1F, skill_penalty);
	} else {
		addSkillLevel(unequippedfrom, 0x1B, skill_penalty);
	}
	return(0x01);
}
