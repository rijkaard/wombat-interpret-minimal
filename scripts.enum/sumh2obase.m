...
function int summon_water_elemental(obj user) {
	int success = 0x00;
	int duration;
	...
	if (!isInMap(there)) {
		...
		list loc_objects;
		getObjectsAt(loc_objects, there);
		if (hasObjVar(this, "magicItemModifier")) {
			...
			if (getSkillLevel(user, SKILL_MAGERY) < 0x0A) {
				...
				duration = 0x14 * getSkillLevel(user, SKILL_MAGERY) / 0x05;
			}
		}
		...
		if (water != NULL()) {
			...
			int pet_init_result = setup_follower(water, user, 0x64, 0x01);
			callback(water, duration, 0x08);
			success = 0x01;
		} else {
