...
function int attempt_spell_cast(obj user, loc usedon, int spell_index, int from_scroll) {
	...
	if (testAndLearnSkill(user, 0x19, cast_time_ms, 0x28) <= 0x00) {
		return(0x00);
	}
...
function int apply_magic_resist_with_learn(obj user, obj usedon, int damage) {
	...
	if (testAndLearnSkill(usedon, 0x1A, damage_to_resist_difficulty(damage), 0x32) > 0x00) {
...
function int apply_magic_resist_no_learn(obj user, obj usedon, int damage) {
	...
	if (roll_skill_check(usedon, 0x1A, damage_to_resist_difficulty(damage), 0x28) > 0x00) {
		systemMessage(usedon, "You feel yourself resisting magical energy!");
		return((damage + 0x01) / 0x02);
	}
	return(damage);
...
function int check_magic_resist_with_learn(obj user, obj usedon, int circle_level) {
	...
	if (testAndLearnSkill(usedon, 0x1A, circle_to_resist_difficulty(circle_level), 0x32) > 0x00) {
		systemMessage(usedon, "You feel yourself resisting magical energy!");
		return(0x01);
	}
	return(0x00);
...
function int test_magic_resist(obj user, obj usedon, int circle_level) {
	...
	if (roll_skill_check(usedon, 0x1A, circle_to_resist_difficulty(circle_level), 0x28) > 0x00) {
		systemMessage(usedon, "You feel yourself resisting magical energy!");
		return(0x01);
	}
	return(0x00);
...
function int scale_damage_by_magery(obj spell_obj, obj caster, int damage) {
	...
	if (isValid(spell_obj)) {
		if (hasObjVar(spell_obj, "magicItemDamage")) {
			...
			if (isValid(caster)) {
				if (isMobile(caster)) {
					skill_bonus = getSkillLevel(caster, 0x19);
				}
			}
		}
	} else {
...
function int get_wall_duration(obj caster, int circle) {
	return(0x0F + ((getSkillLevel(caster, 0x19) * 0x02) / circle));
}
...
function int is_spell_guaranteed_success(obj caster, int circle) {
	if (getSkillSuccessChance(caster, 0x19, get_skill_threshold(circle), 0x28) >= 0x01F4) {
		return(0x01);
	}
	return(0x00);
}
...
function int can_attempt_spell(obj caster, int circle) {
	if (getSkillSuccessChance(caster, 0x19, get_skill_threshold(circle), 0x28) > 0x00) {
		return(0x01);
	}
	return(0x00);
}
