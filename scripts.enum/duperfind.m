...
function int check_item_duped(obj it) {
	if (isMobile(it)) {
		...
		obj bank = getItemAtSlot(it, EQUIP_BANK);
		if (bank != NULL()) {
...
function int has_illegal_stats(obj mobile) {
	...
	int str_mod = abs(getStatMod(mobile, STAT_STR));
	int dex_mod = abs(getStatMod(mobile, STAT_DEX));
	int int_mod = abs(getStatMod(mobile, STAT_INT));
