...
function void animate_if_alive(obj mobile, int anim) {
	if (getItemAtSlot(mobile, EQUIP_MOUNT) != NULL()) {
		return();
	}
...
function obj create_body_part(loc pos, int obj_type, string part_name, string body_name) {
	...
	callbackAdvanced(part, 0x04 * 0x3C * 0x1E, TIMER_EVENT_DELETE, 0x00);
	return(part);
}
...
trigger targetobj {
	...
	case 0x1BE2
		...
		list fletching;
		clearList(fletching);
		int skill_lvl = getSkillLevelReal(user, SKILL_BOWCRAFT) + 0x0190;
		for (int i = 0x00; i < 0x05; i++) {
...
trigger callback(0x6E) {
	...
	if (!testSkill(user, SKILL_LUMBERJACKING)) {
		systemMessage(user, "You hack at the tree for a while, but fail to produce any useable wood.");
...
trigger callback(0x6D) {
	...
	int success = testAndLearnSkill(user, SKILL_BOWCRAFT, getArrayIntElem(0x01, 0x02, arrayIndex), 0x50);
	if (success <= 0x00) {
