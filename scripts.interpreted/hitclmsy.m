...
trigger callback(0x2F) {
	int magic_amt;
	int result = getResource(magic_amt, this, "magic", 0x03, 0x02);
	int charges = magic_amt / 0x03;
...
trigger ishitting {
	...
	if (hasScript(victim, "reflctor")) {
		...
		apply_clumsy_effect(victim, wielder, 0x01);
		detachScript(victim, "reflctor");
	} else {
		apply_clumsy_effect(wielder, victim, 0x00);
	}
