...
trigger targetobj {
	...
	list statics;
	getStaticObjectsAt(statics, getLocation(usedon));
	if (numInList(statics) > 0x01) {
		systemMessage(user, "You can't reach the water there.");
		return(0x00);
	}
	int t = getObjType(usedon);
	int is_water = 0x00;
	if (isInList(water_tile_types, t)) {
		is_water = 0x01;
	}
	if (!is_water) {
...
trigger targetloc {
	...
	int t;
	int is_water = 0x00;
	getStaticObjectsAt(statics, place);
	if (numInList(statics) > 0x01) {
		systemMessage(user, "You can't reach the water there.");
		return(0x00);
	}
